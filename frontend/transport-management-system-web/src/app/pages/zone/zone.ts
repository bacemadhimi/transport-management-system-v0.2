...
export class ZoneComponent implements OnInit {
  ...
  showCols = [
    ...
    {
      key: 'Action',
      //format: (row: IZone) => ["Modifier", "Supprimer"]
       format: (row: IZone) => [this.t('EDIT'), this.t('DELETE')]
    }
  ];
  ...
  getLatestData() {
    this.httpService.getZonesList(this.filter).subscribe({
      next: (result) => {
        this.pagedZoneData = result;
  ...
  // delete(zone: IZone) {
  //   if (confirm(`Voulez-vous vraiment supprimer la zone "${zone.name}" ?`)) {
  //     this.httpService.deleteZone(zone.id).subscribe({
  //       next: () => {
  ...
  delete(zone: IZone) {
  const confirmMessage = this.t('CONFIRM_DELETE_ZONE').replace('{{name}}', zone.name);

  if (confirm(confirmMessage)) {
    this.httpService.deleteZone(zone.id).subscribe({
      next: () => {
        this.showSuccess(this.t('ZONE_DELETED_SUCCESS'));
        this.getLatestData();
      },
      error: (error) => {
        console.error('Error deleting zone:', error);
        this.showError(this.t('ZONE_DELETE_FAILED'));
      }
    });
  }
}
  ...
  exportCSV() {
    const rows: IZone[] = this.pagedZoneData?.data || [];
  ...
  exportExcel() {
    const data: IZone[] = this.pagedZoneData?.data || [];
