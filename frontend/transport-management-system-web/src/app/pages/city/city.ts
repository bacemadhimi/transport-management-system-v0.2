...
export class CityComponent implements OnInit {
  ...
  showCols = [
    ...
    },
    {
      ...
      format: (row: ICity) => [this.t('EDIT'), this.t('DELETE')]
    }
  ];
...
//
  ...
  //       if (confirm(this.t('CONFIRM_DELETE_CITY', { name: city.name }))) {
  //     this.httpService.deleteCity(city.id).subscribe({
  //       next: () => {
  //         //this.showSuccess('ville supprimée avec succès');
  //         this.showSuccess(this.t('CITY_DELETED_SUCCESS'));
  //         this.getLatestData();
  //       },
  //       error: (error) => {
  ...
  //          this.showError(this.t('CITY_DELETE_FAILED'));
  //       }
  //     });
  //   }
  // }
  delete(city: ICity) {
  const confirmMessage = this.t('CONFIRM_DELETE_CITY').replace('{{name}}', city.name);
