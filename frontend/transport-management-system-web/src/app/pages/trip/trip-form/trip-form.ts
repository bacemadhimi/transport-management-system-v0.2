...
export class TripForm implements OnInit {
  ...
  truckZoneFilterControl = new FormControl(null);
  truckZones: any[] = []; 
...
acceptOrdersWithoutAddress: boolean = false;
 ...
 ngOnInit(): void {
  ...
  this.loadAllCustomers().then(() => {
    ...
    this.truckZoneFilterControl.valueChanges
      ...
      .subscribe((zoneId: number | null) => {
        ...
        if (!startDate) {
          Swal.fire({
            icon: 'warning',
            title: 'Sélectionnez une date d\'abord',
        ...
        this.loadTrucksWithZoneFilter(zoneId, startDate);
      });
...
private loadTrucks(): Promise<void> {
  return new Promise((resolve, reject) => {
    const startDate = this.tripForm.get('estimatedStartDate')?.value;
    ...
    const zoneId = this.truckZoneFilterControl.value;
...
private processTruckResponse(response: any, date: Date): void {
  ...
  if (data.availableTrucks && Array.isArray(data.availableTrucks)) {
    ...
    this.availableTrucks = data.availableTrucks.map((apiTruck: any) => {
      ...
      return {
        ...
        capacity: apiTruck.Capacity || apiTruck.capacity || 0,
  ...
  if (data.unavailableTrucks && Array.isArray(data.unavailableTrucks)) {
    ...
    this.unavailableTrucks = data.unavailableTrucks.map((truck: any) => {
      return {
        ...
        capacity: truck.Capacity || truck.capacity || 0,
  ...
  console.log('📊 Final state:', {
    ...
    unavailableTrucks: this.unavailableTrucks
  });
}

private handleCurrentTruckForEdit(response: any): void {
  const truckId = this.tripForm.get('truckId')?.value;
  if (truckId && this.tripId) {
    
    const currentTruckInResponse = response.availableTrucks?.find((t: any) => t.id === truckId) || 
                                   response.unavailableTrucks?.find((t: any) => t.id === truckId);
    
    if (!currentTruckInResponse) {
      
      const currentTruck = this.trucks.find(t => t.id === truckId);
      if (currentTruck) {
        
        const truckWithMessage = {
          ...currentTruck,
          disabled: true,
          availabilityMessage: 'Actuellement assigné à ce voyage',
          tooltip: `Actuellement assigné à ce voyage - Modification en cours`,
          isAvailable: false
        };
        this.trucks.push(truckWithMessage);
        this.unavailableTrucks.push(truckWithMessage);
      }
    }
  }
}

private handleTruckLoadError(date: Date): void {
  console.error('Truck load error - Date:', date);
  ...
  if (isWeekend) {
    this.trucks = this.trucks.map((truck, index) => {
      
      if (index % 3 === 0) {
        return {
          ...truck,
          disabled: true,
          availabilityMessage: 'Non disponible le weekend',
          tooltip: `Indisponible le weekend`,
          isAvailable: false
        };
      }
      return {
...
private checkNoTrucksWarning(date: Date, response: any): void {
  const availableCount = this.availableTrucks.length;
  const unavailableCount = this.unavailableTrucks.length;
  
  if (availableCount === 0 && this.trucks.length > 0) {
    const warningMessage = `Aucun camion disponible le ${this.formatDateForDisplay(date)}`;
    
    if (unavailableCount > 0) {
      this.snackBar.open(
        `${warningMessage} (${unavailableCount} camion(s) indisponible(s))`,
        'Fermer',
        { duration: 5000 }
      );
    }
...
private async showPartialSelectionAlert(
  ...
  const percentageAfter = truck ? (totalAfterAddition / (truck.capacity || 1)) * 100 : 0;
      ...
      <div style="text-align: left;">
        ...
        ${truck ? `
          <div style="background-color: #f3f4f6; padding: 10px; border-radius: 5px; margin: 10px 0;">
            ...
            <p>= Total: ${totalAfterAddition.toFixed(2)} palette / ${truck.capacity} palette</p>
      ...
      html: `
        <div style="text-align: left; padding: 10px;">
          ...
          <p style="color: #ef4444; font-weight: bold;">
            ❌ IMPOSSIBLE D'AJOUTER CES COMMANDES
          </p>
          <p>Veuillez sélectionner moins de commandes ou choisir un camion avec plus de capacité.</p>
        </div>
      `,
      confirmButtonText: 'Compris',
      confirmButtonColor: '#ef4444'
      ...
      html: `
        <div style="text-align: left; padding: 10px;">
          ...
          <p style="color: #f59e0b; font-weight: bold;">
            ⚠️ ATTENTION: Capacité presque pleine
          </p>
...
private loadAvailableDriversByDateAndZone(date: Date, zoneId: number | undefined): void {
  ...
  this.http.getAvailableDriversByDateAndZone(dateStr, zoneId, excludeTripId).subscribe({
    ...
    error: (error) => {
      ...
      this.handleDriverLoadError(date, zoneId, excludeTripId);
      this.loadingAvailableDrivers = false;
    }
  });
}

private handleDriverLoadError(date: Date, zoneId: number | null = null, excludeTripId?: number): void {
  console.error('Driver load error - Date:', date, 'Zone:', zoneId, 'ExcludeTrip:', excludeTripId);
  ...
  if (date) {
    ...
    this.http.getAvailableDriversByDateAndZone('', zoneId, excludeTripId).subscribe({
      next: (response: any) => {
        this.processDriverResponse(response, date);
      },
      error: (fallbackError) => {
...
private refreshDriversByDateAndZone(): void {
  ...
  console.log('Loading drivers for Zone:', zoneId, 'Date:', startDate, 'Excluding trip:', this.tripId);
...
loadDriversByZone(zoneId: number): void {
  ...
  const currentDriverId = this.tripId ? this.tripForm.get('driverId')?.value : null;
  
  this.http.getDriversByZone(zoneId).subscribe({
    next: (drivers: IDriver[]) => {
      if (this.tripId && currentDriverId) {
      ...
      console.log(`Loaded ${drivers.length} drivers for zone ${zoneId} (edit mode: ${this.tripId})`);
    },
    error: (error) => {
...
private suggestAlternativeDriver(startDate: Date, duration: number, excludeTripId?: number): void {
  ...
  this.http.getAvailableDriversByDateAndZone(dateStr, zoneId, finalExcludeTripId).subscribe({
    next: (response: any) => {
...
private loadAvailableDrivers(date: Date | null): void {
  if (!date) {
    ...
    this.loadAllDriversWithZoneFilter();
    return;
  }
...
private loadAllDriversWithZoneFilter(): void {
  const zoneId = !this.zoneFilterDisabled ? this.getStartLocationZoneId() : undefined;
...
getWeatherWarningMessage(): string {
  ...
  private loadZones(): void {
    ...
    const zonesSub = this.http.getActiveZones().subscribe({
      next: (response) => {
...
trackByClientId(index: number, client: any): string {
  ...
  this.http.getActiveZones().subscribe({
    next: (response) => {
      ...
      this.truckZones = zonesData;
    },
    error: (error) => {
      console.error('Error loading truck zones:', error);
      this.truckZones = [];
    }
  });
}

private loadTrucksWithZoneFilter(zoneId: number | null, date: Date): void {
  this.loadingAvailableTrucks = true;
  const dateStr = this.formatDateForAPI(date);
  const excludeTripId = this.tripId;
  ...
  if (!zoneId) {
    ...
    this.http.getAvailableTrucksByDate(dateStr, excludeTripId).subscribe({
      next: (response: any) => {
        this.processTruckResponse(response, date);
        this.loadingAvailableTrucks = false;
      },
      error: (error) => {
    ...
    this.http.getAvailableTrucksByZoneAndDate(zoneId, dateStr, excludeTripId).subscribe({
      next: (response: any) => {
        this.processTruckResponse(response, date);
        this.loadingAvailableTrucks = false;
        ...
        const message = this.availableTrucks.length === 0
          ? `Aucun camion disponible dans la zone ${zoneName}`
          : `${this.availableTrucks.length} camion(s) disponible(s) dans la zone ${zoneName}`;
        
        this.snackBar.open(message, 'Fermer', { duration: 3000 });
      },
      error: (error) => {
        console.error('Error loading trucks by zone:', error);
        ...
        this.http.getAvailableTrucksByDate(dateStr, excludeTripId).subscribe({
          next: (response: any) => {
            ...
            this.availableTrucks = this.availableTrucks.filter(truck => 
              !truck.zoneId || truck.zoneId === zoneId
            );
            this.unavailableTrucks = this.unavailableTrucks.filter(truck => 
              !truck.zoneId || truck.zoneId === zoneId
            );
            this.trucks = [...this.availableTrucks, ...this.unavailableTrucks];
            ...
            const zoneName = this.getZoneName(zoneId);
            const message = this.availableTrucks.length === 0
              ? `Aucun camion dans la zone ${zoneName} (filtrage client)`
              : `${this.availableTrucks.length} camion(s) dans la zone ${zoneName} (filtrage client)`;
...
clearTruckZoneFilter(): void {
  this.truckZoneFilterControl.setValue(null);
}
isTruckInSameZone(truckId: number): boolean {
  const truck = this.trucks.find(t => t.id === truckId);
  if (!truck) return false;
  
  const selectedZoneId = this.truckZoneFilterControl.value;
  if (!selectedZoneId) return false;
  
  return truck.zoneId === selectedZoneId;
}
getTruckZoneTooltip(truck: ITruck): string {
  const selectedZoneId = this.truckZoneFilterControl.value;
  
  if (!selectedZoneId) {
    return truck.zoneId ? `Zone: ${this.getZoneName(truck.zoneId)}` : 'Pas de zone assignée';
  }
  
  const selectedZoneName = this.getZoneName(selectedZoneId);
  const truckZoneName = truck.zoneId ? this.getZoneName(truck.zoneId) : 'Aucune';
  
  if (!truck.zoneId) {
    return `Pas de zone assignée (filtre: ${selectedZoneName})`;
  }
  
  if (truck.zoneId === selectedZoneId) {
    return `Zone correspond au filtre: ${truckZoneName}`;
  } else {
    return `Zone: ${truckZoneName} (filtre: ${selectedZoneName})`;
  }
}
shouldShowZoneMatchBadge(truck: ITruck): boolean {
  const selectedZoneId = this.truckZoneFilterControl.value;
  
  if (!selectedZoneId) return false;
  
  return !truck.zoneId || truck.zoneId === selectedZoneId;
}
getTruckZoneDisplay(truck: ITruck): string {
  if (truck.zoneId) {
    return this.getZoneName(truck.zoneId) || 'Zone inconnue';
  }
  
 
  const selectedZoneId = this.truckZoneFilterControl.value;
  if (selectedZoneId) {
    return 'Aucune zone assignée';
  }
  
  return 'Pas de zone';
}
getTruckZoneIcon(truck: ITruck): string {
  if (truck.zoneId) {
    return 'location_on';
  }
  return 'location_off';
...
getTruckZoneIconColor(truck: ITruck): string {
  const selectedZoneId = this.truckZoneFilterControl.value;
  
  if (!selectedZoneId) {
    return truck.zoneId ? 'primary' : 'accent';
  }
  

  if (!truck.zoneId || truck.zoneId === selectedZoneId) {
    return 'primary'; 
  }
...
private setupAutoSave(): void {
  const debouncedSave = debounceTime(3000); 
...
private saveDraft(): void {
  ...
  try {
    const draft = {
      ...
      truckZoneFilter: this.truckZoneFilterControl.value,
...
calculateClientCapacityPercentage(customerId: number): number {
  ...
  return (clientWeight / truck.capacity) * 100;
}
...
calculateOrderPercentage(orderWeight: number): number {
  ...
  return (orderWeight / truck.capacity) * 100;
}
...
calculateGroupCapacityPercentage(customerId: number): number {
  ...
  return (groupWeight / truck.capacity) * 100;
}
...
private restoreDraftFromStorage(draft: any): void {
  try {
    ...
    if (draft.truckZoneFilter) {
      this.truckZoneFilterControl.setValue(draft.truckZoneFilter);
    }
...
private refreshResourcesForRestoredDraft(draft: any): void {
  ...
  if (startDate) {
    setTimeout(() => {
      ...
      if (draft.truckZoneFilter) {
        setTimeout(() => {
          this.loadTrucksWithZoneFilter(draft.truckZoneFilter, startDate);
        }, 300);
      }
    }, 600);
  }
  ...
  if (startDate && startLocationId) {
    setTimeout(() => {
      this.loadAvailableDrivers(startDate);
      
     
      if (draft.zoneFilter && !this.zoneFilterDisabled) {
        setTimeout(() => {
          const zoneId = this.getStartLocationZoneId();
          if (zoneId) {
            this.loadAvailableDriversByDateAndZone(startDate, zoneId);
...
getSelectedTruckCapacityDisplay(): string {
  ...
  const unit =  this.loadingUnit;
  return `${truck.capacity} ${this.loadingUnit}`;
}
